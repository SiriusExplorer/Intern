...
def mergeRefGeneDf(refgene_df):
    ...
    A large dictionary will be returned with the structure like:
        {name2:[['chr1', [transcript_ID], [exon_region], (TSS, TTS)]
                ['chr2', [transcript_ID], [exon_region], (TSS, TTS)]
                ...
                ['chrX', [transcript_ID], [exon_region], (TSS, TTS)]]
         ...
         name2:...}
    """
    ...
    while i < len(sort_df):
        ...
        while chrom_ind < i+sym_span:
            ...
            for trans_ind in range(chrom_ind, chrom_ind+chrom_span):
                ...
                ex_region_list.extend(parseExon(sort_df.loc[trans_ind]))
...
for gene in merge_dict:
    ...
    for each_chrom in merge_dict[gene]:
        samfetch = samfile.fetch(each_chrom[0], each_chrom[3][0], each_chrom[3][1])
        read_counts += readCount(each_chrom[2], samfetch)
        tmp_matrix = np.array(each_chrom[2])
    ...
    avg_ex_len = exon_length / chrom_num
    RPKM = read_counts * 10**9 / (mapped_reads_amount * avg_ex_len)
